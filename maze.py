...
class Maze:
    ...
    def _break_walls_recursively(self, i, j):
        ...
        while True:
            ...
            self._break_walls_recursively(next_index[0], next_index[1])
    ...
    def _solve_recursively(self, i, j):
        self._animate()

        # go to current cell
        self._cells[i][j].visited = True

        # if its the end return True
        ...
        if i > 0 and not self._cells[i][j].has_left_wall and not self._cells[i - 1][j].visited:
            ...
            if self._solve_recursively(i - 1, j):
                return True
            else:
        ...
        if j > 0 and not self._cells[i][j].has_top_wall and not self._cells[i][j - 1].visited:
            ...
            if self._solve_recursively(i, j - 1):
                return True
            else:
        ...
        # we went the wrong way, return to previous cell by returning False
        return False

    # create the moves for the solution using a depth first search
    def solve(self):
        return self._solve_recursively(0, 0)
